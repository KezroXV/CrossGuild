...
export async function GET(req: Request) {
  try {
    ...
    switch (timeframe) {
      case "day": daysLookback = 1; break;
      case "week": daysLookback = 7; break;
      case "month": daysLookback = 30; break;
      case "quarter": daysLookback = 90; break;
      case "year": daysLookback = 365; break;
    }
    ...
    const inventoryStats = {
      healthyStock: await prisma.item.count({ where: { quantity: { gt: 10 } } }),
      lowStock: await prisma.item.count({ where: { quantity: { gt: 0, lte: 10 } } }),
      outOfStock: await prisma.item.count({ where: { quantity: { equals: 0 } } }),
