...
export async function GET(req: Request) {
  try {
    ...
    const categoryData = Object.entries(salesByCategory).map(([name, value], index) => ({
      name,
    ...
    return NextResponse.json({
      ...
      totalOrders: salesData.reduce((sum: number, day: any) => sum + day.orders, 0),
      totalProfit: salesData.reduce((sum: number, day: any) => sum + day.profit, 0),
    });
...
function getColorByIndex(index: number): string {
  const colors = [
    ...
    "#FF9F40", "#8AC054", "#5D9CEC", "#DADAEB", "#F49AC2"
  ];
