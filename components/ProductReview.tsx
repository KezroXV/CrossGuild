...
const ProductReview = ({ productId }: { productId: string }) => {
  ...
  // Un seul useEffect de debug pour voir ce que contient la session
  useEffect(() => {
    if (session?.user) {
  ...
  useEffect(() => {
    ...
    // Simplifier la mise à jour du formulaire avec l'ID utilisateur
    ...
    try {
      ...
      setReviews(
        response.data.reviews.filter(
          (review: Review) => review.itemId === productId
        )
      );
      ...
      if (session?.user?.id) {
        const userHasReviewed = response.data.reviews.some(
          (review: Review) =>
            review.itemId === productId && review.user.id === session.user.id
        );

        if (userHasReviewed) {
          // Disable the "Write a Review" button functionality by setting a state
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Vérifier directement l'existence de l'ID utilisateur
    ...
    try {
      const reviewData = {
        ...
        userId, // Utiliser directement l'ID utilisateur extrait
        itemId: productId,
      };
